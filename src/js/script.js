$(function() {
  ...
  $('.nav__item').click(function(e) {
    ...
    var target = $(this).attr('href'),
        dest = $(target).find('.section-title').length ? $(target).find('.section-title').offset().top : $(target).offset().top,
        offset = $(target).find('.section-title').length ? $('.header').outerHeight() + 10 : $('.header').outerHeight();
  ...
  $('.fileinput__input').change(function() {
    ...
    if ( false /*files.type !== 'text/xml' && !files.name.includes(".req") && !files.name.includes(".p10")*/) {
  ...
  $('.tabs-nav .tabs-nav__link').click(function(e) {
    ...
    $(this).addClass('tabs-nav__link--active').siblings('.tabs-nav__link').removeClass('tabs-nav__link--active');
    $('.tabs__tab' + $(this).attr('href')).slideDown('100').siblings('.tabs__tab').slideUp();
  });

  // slide toggle
  $('.slide-toggle__head').click(function(e) {
    $(this).toggleClass('slide-toggle__head--active').next().slideToggle('300');
  });
